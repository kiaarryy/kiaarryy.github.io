...
function prepColors(input) {
    const base = (input && input.length ? input : ['#fffaf1', '#d8a23a', '#0c7f78']).slice(0, MAX_COLORS);
    const arr = [];
...
class FerrofluidStage {
    ...
    init() {
        const { arr, count } = prepColors(this.options.colors);
    ...
    loop(time) {
        ...
        const dampening = Math.max(0.0001, this.options.mouseDampening);
        if (!this.lastTime) this.lastTime = time;
        const dt = (time - this.lastTime) / 1000;
        this.lastTime = time;
        const factor = Math.min(1, 1 - Math.exp(-dt / dampening));
        ...
        if (!this.inViewport || this.gatewayHidden || this.documentHidden) return;
...
function initFerrofluid() {
    ...
    document.querySelectorAll('[data-ferrofluid]').forEach((container) => {
        const presetName = container.dataset.ferrofluid;
        const options = presets[presetName];
        if (!options) return;
        try {
            new FerrofluidStage(container, options);
        } catch (error) {
            container.classList.add('is-unavailable');
            console.warn('Ferrofluid effect could not start.', error);
        }
    });
}

if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initFerrofluid, { once: true });
} else {
    initFerrofluid();
}
